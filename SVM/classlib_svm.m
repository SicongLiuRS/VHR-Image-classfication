function [outdata, out_param] = classlib_svm(varargin)
%CLASSIFYSVM Classify with libSVM an image
%
%		[outdata, out_param] = classlib_svm(img, train, opt)
%
% INPUT
...
[np nfeats] = size(data_set);
Ximg = double(data_set);
...
if isempty(cmd)
    [predicted_labels, out_param.accuracy] = svmpredict(ones(np, 1), Ximg, model);
else
    [predicted_labels, out_param.accuracy, out_param.prob_estimates] = svmpredict(ones(np, 1), Ximg, model, cmd);
end

% reshape the array of labels to the original dimensions of the image
outdata = predicted_labels;
%outdata = reshape(predicted_labels,np,1);
