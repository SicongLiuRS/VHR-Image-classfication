...
% ------------------------
if ndims(data_set) ==2
    [npixels nfeats] = size(data_set);
    Ximg = double(data_set);
else
    [nrows ncols nfeats] = size(data_set);
    npixels = nrows*ncols;
    Ximg = double(reshape(data_set, npixels, nfeats));
end
...
if isempty(cmd)
    [predicted_labels, out_param.accuracy] = svmpredict(ones(npixels, 1), Ximg, model);
else
    [predicted_labels, out_param.accuracy, out_param.prob_estimates] = svmpredict(ones(npixels, 1), Ximg, model, cmd);
end
